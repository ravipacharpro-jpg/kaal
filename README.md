...
TUI commands (40 — poori list: [`docs/COMMANDS.md`](docs/COMMANDS.md), palette: Ctrl+P): `/palette` `/dashboard` `/lang` `/setup` `/endpoints` `/platform` `/budget` `/memory` `/agents` `/key` `/model` `/effort` `/ollama` `/schedule` `/perm` `/plan` `/approve` `/review` `/research` `/bg` `/fresh` `/ship` `/autopilot` `/theme` `/session` `/checkpoint` `/rewind` `/tree` `/trace` `/thread` `/export` `/sandbox` `/quit`
